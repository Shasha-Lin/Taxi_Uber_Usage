...
lines = sc.textFile('fhv_*.csv')
lines = lines.mapPartitions(lambda x: reader(x)).filter(lambda x: len(x) >= 1).filter(lambda x: x[1] != 'Pickup_date')
pickup_dates = lines.map(lambda x: x[1]).map(lambda x: (x, base_type(x), semantic_type(x), validate_date(x)))

pickup_dates.saveAsTextFile('v_pickup_dates_fhv.out')
