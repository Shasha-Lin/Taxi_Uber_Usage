...
if __name__ == "__main__":
    ...
    old_green= {'VendorID': 0, 'Pickup_longitude': 5, 'Pickup_latitude': 6, 'Dropoff_longitude': 7, 'Dropoff_latitude': 8, 
    ...
    if old_column_number >= 0:
        if sys.argv[1] == 'yellow':
            taxi_data_old = sc.textFile('old_schema/yellow_*.csv')
        if sys.argv[1] == 'green':
            taxi_data_old = sc.textFile('old_schema/green_*.csv')
        output_old = taxi_data_old.mapPartitions(lambda x: reader(x)).filter(lambda x: len(x) >= (old_column_number + 1)).\
        map(lambda x: x[old_column_number]).\
        map(lambda x: [x, base_type(x), semantic_type(x), 0]).\
        filter(lambda x: x[0] not in column_list).\
        map(lambda x: getattr(current_module, sys.argv[2].lower()(x))).\
        map(lambda x: ','.join()) #should be comma-separated

    if new_column_number >= 0:
        if sys.argv[1] == 'yellow':
            taxi_data_new = sc.textFile('new_schema/yellow_*.csv')
        if sys.argv[1] == 'green':
            taxi_data_new = sc.textFile('new_schema/green_*.csv')
        output_new = taxi_data_new.mapPartitions(lambda x: reader(x)).filter(lambda x: len(x) >= (new_column_number + 1)).\
        map(lambda x: x[new_column_number]).\
        map(lambda x: [x, base_type(x), semantic_type(x), 0]).\
        filter(lambda x: x[0] not in column_list).\
        map(lambda x: getattr(current_module, sys.argv[2].lower()(x))).\
        map(lambda x: ','.join())
